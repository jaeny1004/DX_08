          ...
          >
            ...
            <div className="flex max-w-xs rounded-2xl border border-slate-200 bg-slate-100 p-1 text-sm font-bold text-slate-600">
              <button
                ...
                className={`flex-1 rounded-xl py-2 transition-all ${reportMode === "create"
              ...
              <button
                ...
                className={`flex-1 rounded-xl py-2 transition-all ${reportMode === "browse"
                  ? "bg-white text-emerald-950 shadow-sm"
                  : "hover:text-slate-900"
                  }`}
              >
                        ...
                        {(Object.keys(REPORT_TYPE_LABELS) as ReportType[]).map(
                          (type) => (
                            <button
                              ...
                              className={`rounded-xl border px-2 py-2.5 font-bold transition ${reportType === type
                        ...
                        {(["pdf", "docx", "xlsx"] as ReportFormat[]).map(
                          (itemFormat) => (
                            <button
                              ...
                              className={`rounded-lg border py-2 font-bold ${format === itemFormat
                                ? "border-emerald-800 bg-emerald-800 text-white"
                                : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                                }`}
                            >
            ...
            <div className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 text-xs font-semibold shadow-sm lg:col-span-5">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  해당 격자의 입지(기후대·산림토양형·토심)와 임상 수종구성,
                  같은 시군구의 실제 활엽수 분포를 근거로 대체 수종과 조림 방향을
                  제시합니다.
                </p>
              </div>
